...
public class PublishRegisterUserWithExchange {
    ...
    public boolean publishMessage(UserRegistrationMessage message) {
        try(var channel = connection.createChannel()) {
            ...
            channel.basicPublish(Constants.DirectExchange, Constants.RegisterUserWithExchangeRoutingKey, properties, jsonFormattedMessage);

            System.out.println(" [*] Published (1): " + message);

            channel.basicPublish(Constants.DirectExchange, Constants.RegisterUserWithExchangeAnotherRoutingKey, properties, jsonFormattedMessage);
